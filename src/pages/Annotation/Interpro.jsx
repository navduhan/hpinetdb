...
export default class GO extends React.Component {
  ...
  fetchResults() {
    axios
      .get(
        `${env.BACKEND}/api/interpro/?species=${species}&sptype=${sptype}&page=${this.state.currentPage}&size=${this.state.perPage}`
      )
      .then((res) => {
    ...
    return (
      <div className="container">
        ...
        <div className="row flex-lg-row justify-content-center g-2 my-2">
          <h5>Functional Domains of <i>{data[species]}</i></h5>
          <Divider />
        </div>
        <div className="row flex-lg-row align-items-center g-2 my-2">
          <h5>
            ...
            {this.state.total} Functional Domains
          </h5>
        </div>
