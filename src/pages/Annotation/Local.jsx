...
export default class GO extends React.Component {
  ...
  fetchResults() {
    axios
      .get(
        `${env.BACKEND}/api/local/?species=${species}&sptype=${sptype}&page=${this.state.currentPage}&size=${this.state.perPage}`
      )
      .then((res) => {
    ...
    return (
      <div className="container">
        ...
        <div className="row flex-lg-row justify-content-center g-2 my-2">
          <h5>Sub-cellular Localization of <i>{data[species]}</i> </h5>
          <Divider />
        </div>
